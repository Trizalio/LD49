...
float discretize(float val, int steps){
	return round(val * float(steps)) / float(steps);
}
...
void fragment(){
	...
	int steps = 100;
	vec2 UV_ = vec2(discretize(UV.x, steps), discretize(UV.y, steps));
	vec2 local_position = UV_ * real_size - vec2(extension) ;
	vec2 uv = local_position / texture_size / 1.;
	float current_power = power * round(0.5 + sin(TIME * pulse_frequency * 2. * 3.14));
	...
	if (COLOR.a < 1.){
		...
		COLOR.a = round(fade_factor) * selected_glow.a;
	}
//	if (uv.x < 0.){
//		COLOR.a = 0.;
//	}
